// Dashboard Empleado - Panel personal de asistencias
// Muestra estado diario, turnos, historial y resumen de pagos
    ...
    try {
        ...
        if (data.nextShifts && data.nextShifts.length > 0) {
            data.nextShifts.forEach(shift => {
                ...
                // Usar formateo manual para evitar desfase de zona horaria (new Date() usa UTC)
                const [year, month, day] = shift.fecha.split('-');
