...
/**
 * Función de utilidad para formatear cadenas de hora (ISO o HH:MM:SS) a HH:MM.
 * Evita el error 'Invalid Date' en navegadores móviles al usar cadenas de tiempo MySQL.
 * @param {string} timeValue - Cadena de fecha/hora ISO o cadena de hora HH:MM:SS.
 */
function formatTime(timeValue) { 
    ...
    // Si la DB devuelve un string de tiempo (ej: "10:30:00")
    if (typeof timeValue === 'string' && timeValue.match(/^\d{2}:\d{2}:\d{2}/)) {
        // Devolvemos solo HH:MM
        return timeValue.slice(0, 5); 
    }
    
    // Si es un objeto Date o una cadena ISO completa, usamos el formateador de fechas
    try {
        const date = new Date(timeValue);
        // Usamos una verificación más estricta para asegurar que la fecha sea válida
...
/**
 * Se ejecuta al escanear un código QR con éxito y llama a la API.
 * @param {string} decodedText - El contenido del QR.
 */
...
        
        // Recargar datos del dashboard después de marcar
        fetchDashboardData(); 
...
/**
 * Inicia el escáner forzando el modo de cámara (trasera o frontal).
 * @param {string} mode - 'environment' (trasera) o 'user' (frontal).
 */
...
/**
 * Carga todos los datos del dashboard del empleado (asistencia de hoy, historial y turnos)
 */
